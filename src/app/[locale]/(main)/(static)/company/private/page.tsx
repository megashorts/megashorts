...
// 마크다운 파일 읽기 함수
async function getPrivateContent(locale: string = 'ko') {
  try {
    const filePath = path.join(process.cwd(), `public/locales/${locale}/private.md`);
    const content = await fs.promises.readFile(filePath, 'utf-8');
    return content;
  } catch (error) {
    // 에러 시 한국어 파일로 폴백
    try {
      const fallbackPath = path.join(process.cwd(), 'public/locales/ko/private.md');
