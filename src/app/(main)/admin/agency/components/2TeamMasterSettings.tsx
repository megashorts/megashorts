...
//                       // 검증 후 동기화 호출
//                       const result = await validateAndSyncReferralStructure(selectedUser.id, {}, selectedUser.username);
                      
//                       // 개발자 콘솔에 상세 정보 로깅 (기존 로깅 유지)
//                       console.log("동기화 검증 결과:", result);
                      
//                       // 결과가 없거나 응답이 없는 경우 오류 메시지 표시
//                       if (!result || !result.success) {
//                         console.error("동기화 응답 오류:", result);
                        
//                         // 오류 메시지 추출
//                         let errorMessage = "추천인 구조 동기화 중 오류가 발생했습니다.";
...
//                           new Notification("추천인 구조 동기화 실패", {
//                             body: errorMessage,
//                             icon: "/favicon.ico"
//                           });
//                         }
...
//                       // 검증 결과 확인 (result.result가 없는 경우도 처리)
//                       if (!result.result) {
//                         console.error("동기화 결과 없음:", result);
                        
//                         // 브라우저 알림 표시
//                         if (Notification.permission === "granted") {
//                           new Notification("추천인 구조 동기화 실패", {
//                             body: "추천인 구조 동기화 중 오류가 발생했습니다. 결과 데이터가 없습니다.",
//                             icon: "/favicon.ico"
//                           });
//                         }
...
//                         // 토스트 메시지 표시
//                         toast({
//                           description: "추천인 구조 동기화 중 오류가 발생했습니다. 결과 데이터가 없습니다.",
//                           variant: "destructive",
//                           duration: 3000,
//                         });
                        
//                         // 알림창 표시
//                         alert("추천인 구조 동기화 중 오류가 발생했습니다. 결과 데이터가 없습니다.");
//                         return;
//                       }
...
//                         // 검증 실패 시 사용자에게 확인 요청
//                         const issues = result.result.validationResult?.issues || [];
//                         const issueCount = issues.length;
//                         const issuesDetails = result.result.issuesDetails || '상세 내용 없음';
                        
//                         // 문제가 있는 경우 동기화 불가능 메시지 표시
//                         if (result.result.cannotSync) {
//                           console.error("동기화 실패:", result.result);
                          
//                           // 토스트 메시지 표시
//                           toast({
//                             description: result.result.errorMessage || "추천인 구조에 문제가 있어 동기화할 수 없습니다.",
//                             variant: "destructive",
//                             duration: 3000,
//                           });
                          
//                           // 알림창 표시 (개발자 콘솔에 있는 상세 오류 정보 포함)
//                           alert(`추천인 구조 동기화 실패: ${result.result.errorMessage || "추천인 구조에 문제가 있어 동기화할 수 없습니다."}\n\n상세 문제: ${issuesDetails}`);
//                       } else {
//                         // 확인 대화상자 표시 (문제 상세 내용 포함)
//                         const issuesDetails = result.result.issuesDetails || '상세 내용 없음';
                        
//                         if (confirm(result.result.confirmationMessage || `추천인 구조에 ${issueCount}개의 문제가 발견되었습니다: ${issuesDetails}. 그래도 동기화를 진행하시겠습니까?`)) {
//                           try {
//                             // 사용자가 확인한 경우 일반 동기화 수행
//                             const syncResult = await syncReferralStructure(selectedUser.id, {}, selectedUser.username);
                            
//                             if (syncResult.success && syncResult.updated) {
//                               toast({
//                                 description: "추천인 구조 동기화가 완료되었습니다.",
//                                 variant: "default",
//                                 duration: 1500,
//                               });
//                             } else {
//                               console.error("동기화 실패:", syncResult);
//                               toast({
//                                 description: syncResult.message || "추천인 구조 동기화 중 오류가 발생했습니다.",
//                                 variant: "destructive",
//                                 duration: 3000,
//                               });
//                             }
...
//                             toast({
//                               description: "추천인 구조 동기화 중 오류가 발생했습니다.",
//                               variant: "destructive",
//                               duration: 3000,
//                             });
//                           }
//                         } else {
//                           toast({
//                             description: "추천인 구조 동기화가 취소되었습니다.",
//                             variant: "default",
//                             duration: 1500,
//                           });
//                         }
...
//                       }
//                     } catch (error) {
//                       console.error("추천인 구조 동기화 중 오류가 발생했습니다:", error);
//                       toast({
//                         description: "추천인 구조 동기화 중 오류가 발생했습니다.",
//                         variant: "destructive",
//                         duration: 1500,
...
//                     }
//                   }}
//                   disabled={syncLoading || !selectedUser}
//                 >
//                   {syncLoading ? (
//                     <>
